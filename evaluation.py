...
TAR_DIR = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)), './data/test_y.zip'))
PRED_DIR = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)), './data/test_x.zip'))
...
def evaluate(path_to_src_dir):
    ...
    for i in range(NUM_MAX_RUNS):
        ...
        #save files
        forecast_file_path = os.path.join(path_to_src_dir, "prediction.csv")
        real_file_path = os.path.join(path_to_src_dir, "real.csv")
        pd.DataFrame(np.squeeze(prediction)).to_csv(forecast_file_path)
        pd.DataFrame(np.squeeze(gt_ys)).to_csv(real_file_path)
